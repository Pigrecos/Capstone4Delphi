...
{$else}
  {$ifdef CPUX64} 
    LIB_FILE = 'CapStone\capstone_x64.dll';
  {$else}
     LIB_FILE = 'CapStone\capstone_x32.dll';
  {$endif}   
{$endif}
...
type
  ...
  const
	 ...
	 CS_AC_READ    = 1 Shl 0;   // Operand read from memory or register.
	 CS_AC_WRITE   = 1 Shl 1;    // Operand write to memory or register.
...
function cs_option(handle: csh; _type: Cardinal; value: Cardinal): cs_err; cdecl external LIB_FILE;
...
function cs_disasm_iter(handle: csh; var code: Pointer; var size: NativeUInt;  var address: UInt64; insn: Pcs_insn): boolean; cdecl external LIB_FILE;
...
*}
type
  cs_regs   = array [0..63] of Char ;
  Pcs_regs  = PWideChar;

function cs_regs_access(handle: csh; const insn: Pcs_insn; regs_read: pcs_regs; regs_read_count: Pointer; regs_write: pcs_regs; regs_write_count: Pointer): integer; cdecl external LIB_FILE;
...
function X86_REL_ADDR(insn: cs_insn): UInt64;
begin
	Result := insn.address + insn.size + uint32(insn.detail^.x86.disp);
end;
